						...
						IF ROUND((_children / 2), 2) != 0.00 AND _position = 'right, left'
							THEN
								...
								THEN
									IF newCount != upCount
										THEN
											CALL  add_userCommission(userId, 60, 'upline', userId, CONCAT('depth_', userId, '-',_depth));
									END IF;
                                END IF;
							...
							THEN
                                IF oldCount != 0 AND newCount != 0 AND newCount != oldCount AND newCount != upCount
									THEN
										IF newCount != upCount
											THEN
												CALL  add_userCommission(userId, 60, 'upline', null, CONCAT('depth_', userId, '-',_depth));
...
-- Dump completed on 2017-03-03 23:24:24
