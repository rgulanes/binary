...
--
-- Dumping routines for database 'binary'
--
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `add_userCommission`(userId INT, amount FLOAT, _desc VARCHAR(45), r_userId INT, _depth VARCHAR(11)) RETURNS varchar(11) CHARSET latin1
BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `get_Position`(`childId` INT(11)) RETURNS varchar(11) CHARSET latin1
BEGIN
...
BEGIN
    ...
    DECLARE cnt INT(11);
    DECLARE generateCommission INT(11);
    
    SET iCommission = (SELECT COUNT(*) FROM commission WHERE c_user_id = userId AND depth = _depth LIMIT 1);
    SET generateCommission = (SELECT COUNT(*) FROM commission WHERE c_user_id = userId AND DATE(date_create) = DATE(NOW()) LIMIT 1);
    SET maxChild = POW(2, ( _depth));
    
    IF generateCommission <= 10
		THEN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `generateCommissionByPosition` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `generateCommissionByPosition`(IN userId INT(11))
BEGIN
    DECLARE cnt INT(11);
    DECLARE generateCommission INT(11);
    ...
    DECLARE iCommission INT(11);
    DECLARE cLevel INT(11);
    DECLARE totPairs INT(11);
    DECLARE totCommissions INT(11);
    
    CALL get_Hierarchy(userId);
    
    SET lCount = (SELECT COUNT(*) FROM _selectedhierarchy WHERE f_position = 'left');
    SET rCount = (SELECT COUNT(*) FROM _selectedhierarchy WHERE f_position = 'right');
    SET iCommission = (SELECT COUNT(*) FROM commission WHERE c_user_id = userId AND r_user_id = userId AND remarks = 'upline');
    SET generateCommission = (SELECT COUNT(*) FROM commission WHERE c_user_id = userId AND remarks = 'upline' AND DATE(date_create) = DATE(NOW()) LIMIT 1);
    
    SET totPairs = (lCount + rCount);
    SET cLevel = (totPairs / 2);
    
    SET totCommissions = FLOOR((cLevel - iCommission));
    
    IF generateCommission <= 10
		THEN
			SET cnt = 1;
			WHILE cnt <= totCommissions DO
				INSERT INTO commission (c_user_id, c_amount, r_user_id, depth, remarks, date_create)
					VALUES(userId, 60, userId, CONCAT('pairing_', userId), 'upline', NOW());
                SET cnt = cnt + 1;
			END WHILE;
	END IF;
END ;;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `generateLuyabayaTree` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_HierarchyByDate` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_HierarchyByDate`(IN userId INT, IN dateString VARCHAR(10))
BEGIN
	DROP TEMPORARY TABLE IF EXISTS _hierarchy;
    CREATE TEMPORARY TABLE IF NOT EXISTS _hierarchy AS (
	SELECT 
		...
		h.parent AS m_parent
			FROM hierarchy h
	WHERE h.parent = userId AND DATE(h.datetime) <= dateString);
...
-- Dump completed on 2017-03-28 23:52:23
